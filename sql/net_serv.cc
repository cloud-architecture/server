...
static int parse_proxy_protocol_v1_header( char *hdr, size_t len,
  struct sockaddr_storage *addr_storage, int *port, bool *local_connection)
{
  char *ctx = NULL;
  char *token;
  int   address_family;
  ...
  *local_connection = false;
  ...
  addr_storage->ss_family = address_family;
  ...
  *port = atoi(token);
  ...
  if (address_family == AF_INET)
    ((struct sockaddr_in *)addr_storage)->sin_port= (ushort) *port;
  else
    ((struct sockaddr_in6 *)addr_storage)->sin6_port = (ushort) *port;
...
static int parse_proxy_protocol_v2_header(char *hdr, size_t len,
  struct sockaddr_storage *addr_storage,
  int *port, bool *local_connection)
{
  *local_connection = false;
  /* V2 Signature */
  ...
  {
  case 0x11:  /* TCPv4 */
    sin = (struct sockaddr_in *)addr_storage;
    sin->sin_family = AF_INET;
    memcpy(&(sin->sin_addr.s_addr), hdr + 16, 4);
    *port = ((uchar)hdr[24] << 8) + (uchar)hdr[25];
    break;
  case 0x21:  /* TCPv6 */
    sin6 = (struct sockaddr_in6 *)addr_storage;
    sin6->sin6_family = AF_INET6;
    memcpy(&(sin6->sin6_addr), hdr + 16, 16);
    *port = ((uchar)hdr[48] << 8) + (uchar)hdr[49];
    break;
  default:
    *local_connection = true;
  }
...
static int parse_proxy_protocol_header(NET *net)
{
  char hdr[108];
  size_t hdr_len = 0;
  ...
  if (is_v1_header)
  {
    ...
    if (parse_proxy_protocol_v1_header(hdr, hdr_len,  &addr, &port, &local_connection))
      return -1;
  }
  else // if (is_v2_header)
  {
    ...
    if (parse_proxy_protocol_v2_header(hdr, hdr_len,  &addr, &port, &local_connection))
      return -1;
  }
  if (local_connection)
    return 0;
  ...
  char ip_string[128];
  if (vio_get_normalized_ip_string((struct sockaddr*)&addr, sizeof(addr), ip_string, sizeof(ip_string)))
    return -1;
  return thd_set_peer_addr((THD *)net->thd, &addr, ip_string, port);
}
...
packets_out_of_order:
  {
    if (!parse_proxy_protocol_header(net))
    {
      goto retry;
    }
